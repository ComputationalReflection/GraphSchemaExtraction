...
public class GetInfo {
    ...
    private String getValueInString(PropertyTypes propertyType, Object propertyObj) {
        switch (propertyType) {
            case INTEGER:
            case FLOAT:
            case STRING:
    ...
    private Long durationValueRemoteToLong(org.neo4j.driver.internal.value.DurationValue durationValue) {
        IsoDuration duration = (durationValue).asIsoDuration();
        return duration.days() * 24 * 3600 + duration.seconds();
    }

    private Long durationValueLocalToLong(DurationValue durationValue) {
